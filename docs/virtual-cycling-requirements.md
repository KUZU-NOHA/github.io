...
実機（HITFIT_19403）での検証を経て、以下の3点を追加した。いずれも
「標高データにはノイズが乗る」という前提から、送信する勾配を安定させることが目的。

1. **標高データの平滑化**: Elevation API・GPX 由来の標高は数十cm〜数m単位でばらつく。
   `smoothElevations()`（`app/js/map/geo.js`）で移動平均をかけてから勾配計算に使う
2. **勾配自体の平滑化と変化率制限**: `gradeAt()` は前後40m窓で局所平均を取っているが、
   走行位置がわずかに動くだけでも値が揺れうる。`RideEngine` 内で指数移動平均
   （時定数1.5秒・変化率上限6%/秒）を通してから HUD 表示とトレーナー送信の両方に使う。
   壁時計ではなくシミュレーション時間（`elapsedSec`）を基準にしており、
   フレームレートのブレやテスト実行に依存しない
3. **車種プロファイルの反映**: 転がり抵抗係数(crr)・空気抵抗係数(Wind Resistance
   Coefficient)を、走行速度の計算に使っているのと同じ車種プロファイル
   （第6.3節 F-303 参照）から導出して送るようにした。従来は固定のデフォルト値
   （crr=0.004, cw=0.51）を常に送っており、車種を変えても速度計算には反映されるのに
   トレーナー側の負荷計算には反映されないという不整合があった。
   - crr はプロファイルの値をそのまま使う（無次元で単位の対応が明確なため）
   - cw（Wind Resistance Coefficient）は FTMS 仕様上 kg/m 単位で定義されているが、
     実装によって絶対値の解釈（`0.5×Cd×A×ρ` を送るか `Cd×A` を送るか）が割れており、
     仕様書のみからは断定できない。誤った絶対値を送るリスクを避けるため、
     多くの FTMS 実装がデフォルトとして使う `0.51` を基準点に固定し、
     ロードバイクを基準にした車種間の相対比率のみを反映する
     （`trainerWindResistance()`、`app/js/ride/physics.js`）
...
| **Phase 4**<br>体験向上 | 勾配連動の精緻化（✅完了）、Street View チェックポイント（✅完了）、ゴースト走行（✅完了）、ルートライブラリ（✅完了） | ✅ 全項目完了 |
