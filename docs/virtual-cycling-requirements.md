...
#### F-104 プリセットルートの道路スナップ（実装済み）

プリセット（皇居一周・大阪城公園等）の座標は手作業による粗い近似（10〜13点程度）で作成しており、そのまま直線で結んで表示すると**建物や川を突っ切ってしまう**不具合があった（実機検証で発見）。

対応として、プリセットの概算座標を Routes API (`computeRoutes`) の経由地点（`intermediates`）として渡し、実際の道路に沿った経路を取得し直す `routeFromPresetRefined()` を実装した（`app/js/map/route.js`）。

- API キーがある場合: プリセットの始点・終点を `origin`/`destination`、中間の点（最大20点に間引き）を `intermediates` として Routes API に渡し、`BICYCLE → WALK → DRIVE` の自動フォールバック（F-102）付きで実際の道路にスナップした経路を取得する
- API キーが無い場合、または取得に失敗した場合: 従来どおりの概算座標（直線結び）にフォールバックし、「実際の道路とズレる場合がある」旨の警告を表示する
- `BICYCLE` モードで取得できた場合も、自転車ルートは beta 提供のため警告表示義務（L-05）は変わらず適用される
