...
| P-04 | **例外（サブスク利用時のみ）**: メールアドレスとライセンスキーは、有効性確認のためバックエンド（決済代行の Lemon Squeezy を含む）に送信される。P-01 の対象（体重・体脂肪率・心拍・走行記録）はこの例外に含まれず、サブスク利用時も引き続き送信されない |
...
`server/`（このリポジトリ内、`app/` とは別に Vercel へ単独デプロイ）に4つのエンドポイントを持つ。
...
いずれも共通のミドルウェア（`server/lib/requireLicense.js`）でライセンスキーの有効性を確認してから処理する（未設定なら401、無効なら403）。Google Cloud 側では、系統A用・系統B用に**別々のAPIキーを2本発行する**（系統Aはサーバー間通信のため Referer ヘッダーが付かず、リファラ制限キーと併用できないため）。

#### 7.5.3 決済

決済・税務・解約・ライセンスキー発行は **Lemon Squeezy**（Merchant of Record）に委ねる。理由:

- 自前で Webhook 処理・サブスクDB・督促(dunning)ロジックを実装せずに済む（個人開発者が決済基盤を初めて持つ際の一番の事故要因）
- ライセンスキーAPI（発行・有効化・検証・無効化）を標準搭載しており、1キーあたりの有効化上限で共有・使い回しを抑制できる
- 手数料 5% + $0.50/件。2024年に Stripe に買収され基盤は Stripe（事業継続リスクが低い）

利用者はメールアドレスで購入し、届いたライセンスキーを設定画面（メールアドレス欄＋ライセンスキー欄）に貼ることで有効化する。
...
| R-11 | サブスク基盤（Vercel/Lemon Squeezy）の障害・未デプロイ | 中 | BYOK への自動フォールバック。無料導線（プリセットGPX・BLE等）は影響を受けない（第7.5.4節） |
...
| 5 | **サブスク価格・特定商取引法上の事業者情報（氏名/屋号・住所・返金ポリシー）は未確定**。`docs/legal/` にテンプレートを用意し、実データの確定と Lemon Squeezy 側の商品登録（Phase B）を待っている | 第7.5節、`docs/legal/` |
