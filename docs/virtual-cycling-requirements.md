...
1. プロジェクト概要
1.1 背景と目的
室内でエアロバイク／固定ローラーを漕ぐ運動は、ダイエットの手段として有効だが極めて退屈であり、continuity（継続性）が最大の課題となる。本アプリは、実在する世界の風景を Google Maps の 3D 地図データで再現し、自分がペダルを漕いだ分だけ景色が進む体験を提供することで、この退屈さを解消する。
1.2 ゴール
「ダイエットのための有酸素運動を、飽きずに継続できる状態にする」
1.3 最重要 KPI
本アプリの成否は消費カロリーの絶対量ではなく、継続率で測る。
KPI	目標値	測定方法
...
iOS は Safari も iOS版Chrome も Web Bluetooth API に非対応である。iOS版Chrome は名前こそ Chrome だが、Apple の規約により描画エンジンが WebKit に強制されるため、Safari と全く同じ制約を受ける。Apple はプライバシー・セキュリティ上の懸念を理由に、Web Bluetooth の実装を明確に拒否している（Firefox も同様）。
...
Photorealistic 3D Tiles だけが「セッション課金」である。 1回のセッション（3時間有効）を開始すれば、その間のタイル取得は何回でも追加課金されない。つまり1回のライド全体が実質1リクエスト分で済む。月1,000セッションの無料枠があるため、毎日乗っても実質無料である。
...
B. Web アプリ + WebBLE ブラウザ	App Store の `WebBLE`（iOS の CoreBluetooth を Web Bluetooth API に橋渡しする WKWebView ベースの専用ブラウザ）で同じ Web アプリを開く	数百円（アプリ購入）	ゼロ（案Aと同一コード）	iPhone で使える。ただし API は部分実装のため実機検証が必須
...
送信頻度の要件: BLE の書き込みを過剰に行うと接続が不安定になるため、2秒ごと、または勾配変化が 0.5% を超えたときのみ送信する。
...
要件: 接続時に Fitness Machine Feature `0x2ACC` を読み取り、取得可能なデータに応じて機能を動的に有効化すること。
...
擬似的な速度・ケイデンス・パワーを生成するモードを実装する。これは以下の理由から「あれば便利」ではなく必須要件である。
...
強度は速度・ケイデンスから推定する。推定値である旨を UI 上に明示すること（パワー方式との精度差をユーザーに誤認させないため）。
F-505 心拍ゾーン定義
...
S-02	Google Cloud コンソールで HTTP リファラ制限を設定（`kuzu-noha.github.io/*` のみ許可）
S-03	API 種別制限を設定（使用する API のみに限定）
S-04	日次クォータ上限を設定（想定利用量の2〜3倍程度で頭打ちにする）
S-05	予算アラートを設定（意図しない課金の早期検知）
...
P-02	初期リリースでは一切サーバーに送信せず、ブラウザのローカルに閉じる
P-03	データのエクスポート・全削除機能を提供する
7.5 可用性・耐障害性
ID	要件
A-01	走行中に通信が切断されても、記録は継続する（映像のみ静止）
...
L-05	自転車ルートは beta 提供	自転車ルート表示時に、経路が不正確な可能性がある旨の警告文の表示義務がある
...
Phase 1<br>MVP	ルート選択 → 走行 → HUD表示 → セッション記録 の一連が通る	1本のルートを最後まで走って記録が残る
Phase 2<br>ダイエット機能	体重記録・カロリー収支・グラフ・目標管理	ダッシュボードで週次の収支が見える
Phase 3<br>iPhone対応	WebBLE で実機検証 → 不足ならネイティブ化を判断	iPhone で走行できる／または判断結果が出る
Phase 4<br>体験向上	勾配連動の精緻化、Street View チェックポイント、ゴースト走行、ルートライブラリ	—
...
合計	¥45,000〜¥162,000	
利点:
パワー（W）が取得でき、消費カロリーが推定ではなく実測ベースになる（kJ ≒ kcal）
...
確認したい事項
お持ちの機材（スマートトレーナー / センサー / 心拍計 / まだ無い）
PC/Mac をバイクの前に置ける環境か（置けるなら Phase 1 がそのまま実用になる）
主に走りたいエリア（3Dタイルのカバレッジ確認のため）
---
13. 参考資料
...
3D Maps — Maps JavaScript API
Animate camera paths
Google Maps Platform Terms of Service
Google Maps Platform Service Specific Terms
Policies for Street View Static API
Pricing categories
Bluetooth
Fitness Machine Service 1.0 — Bluetooth SIG
Cycling Speed and Cadence Service
Indoor Bike Data characteristic 定義（GATT XML）
Web Bluetooth API — MDN
Bluetooth API — Can I use
WebBLE — iOS WebBluetooth Polyfill
カロリー換算
Calories and Power — TrainerRoad
Energy Expenditure: Calories, Kilojoules, and Power in Cycling — CTS
競合調査
Rouvy — Wikipedia
Best indoor cycling apps 2026 — Cyclist
