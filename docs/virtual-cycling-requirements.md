...
- ログイン・アカウント管理（メールアドレス＋パスワード等によるユーザーDB、複数端末間の同期）

> **2026-08 追記**: 当初は「サーバーサイドの課金」自体をスコープ外としていたが、多くの人に使ってもらうための導線として**軽量なサブスクリプション課金**を追加した（第7.5節）。これは「ライセンスキーの有効性をバックエンドで照会するだけ」の仕組みであり、上記のログイン・アカウントDBを持つ本格的なユーザー管理システムとは別物。**自分の API キーを設定画面に貼って使う無料の方法（BYOK）は、この変更後も無期限で維持する。**
...
3D 表示が使えない場合のフォールバックは、当初は自前 Canvas によるルート線・現在地マーカーのみの簡素な表示だった。**API キーがある場合は、Static Maps API で取得した実際の地図画像を背景に敷く**ことで、3D が使えない環境でも地形・道路が見える表示にした（`app/js/map/fallback2d.js`）。

- Static Maps API に `center` / `zoom` を明示指定し、Canvas 側の投影（Web Mercator）を全く同じ計算式で行うことで、背景画像とオーバーレイ（ルート線・現在地マーカー）を正確に一致させている。`path` パラメータによる自動フィットは余白の取り方が非公開で位置がずれるため使っていない
- 画像はルート選択時に1回だけ取得し、走行中は再取得しない（低コスト。Static Maps API は $2/1,000、月10,000無料）
- 画像取得に失敗しても、従来どおりの図形描画のみの表示にフォールバックする
...
1. **標高データの平滑化**: Elevation API・GPX 由来の標高は数十cm〜数m単位でばらつく。
   `smoothElevationsByDistance()`（`app/js/map/geo.js`）で移動平均をかけてから勾配計算に使う。
   GPX の記録点間隔は低速時・旋回時に密集し高速時に疎になるなど大きく不均一なため、
   点数ベースではなく実距離200m分の点を集めて平均する。
   > **実機以降に判明した追加の限界**: GPS標高は都心部の建物・樹木等の影響で、
   > 単発ノイズではなく**数百m単位でまとまった誤差**が乗ることがある（皇居ルートで
   > 実測、数十m規模の見せかけの下り坂として現れた）。この規模の誤差は現実的な
   > 平滑化窓では吸収しきれないため、`gradeAt()` の勾配クランプを ±25% → **±15%**
   > に強化し、Wahoo 独自プロトコルの安全側クランプ（`buildSimGradeCommand()`）も
   > 合わせて ±15% にした。実在する山岳ルート（寒風山・男鹿半島など）の勾配は
   > 大半が±15%に収まっているため、これによる実データへの影響は小さい。
...
| F-104 | プリセットルート（実際に記録された GPX ファイルを数本同梱） | 高 |
...
#### F-104 プリセットルート（実装済み・GPX 同梱方式）

プリセットは当初、手作業で座標を10〜13点ほど近似して直線で結ぶ方式だった。これは**建物や川を突っ切ってしまう**不具合を起こした（実機検証で発見）。座標をより細かくする改良や、Google Routes API の経由地点機能で道路にスナップさせ直す改良も試みたが、後者は「始点=終点となるループルートで経路計算エンジンが経由地点を無視し、施設内を突っ切る退化した結果を返すことがある」という別の不具合を生んだ。

最終的に、**Ride with GPS で実際に記録・計画された GPX ファイルをそのまま `app/routes/*.gpx` に同梱し、プリセット選択時に読み込む方式**に切り替えた（`routeFromPresetGpx()`、`app/js/map/route.js`）。実際に走った・計画されたルートそのものなので、道路との整合性やループの閉じ方に関する不具合が構造的に起こらない。GPX に標高も含まれるため、Elevation API も不要になり、API キー無しでも正確な経路・標高でプリセットが使える。

| id | ルート | 距離 | ループ |
|---|---|---|---|
| `imperial-palace` | 皇居・東京タワー（東京） | 約9.7km | - |
| `osaka-castle` | 大阪城天守閣＋標高0m（大阪） | 約7.7km | - |
| `golden-gate` | ゴールデンゲートブリッジ（サンフランシスコ） | 約20km | - |
| `seine` | セーヌ川（パリ→河口方面） | 約287km | - |
| `oga-peninsula` | 寒風山・男鹿半島（秋田） | 約166km | - |
| `nihonbashi-shinagawa` | 日本橋・品川（東京） | 約7.9km | - |
| `shibuya-aoyama` | 渋谷・青山（東京） | 約8.0km | ○ |
| `yaesu-kokyo-loop` | 八重洲口・皇居一周（東京） | 約7.0km | ○ |

> GPX ファイルはユーザー本人が Ride with GPS で作成したルートの提供を受けて同梱している。ファイルには経路の緯度経度・標高に加えて、Ride with GPS 上のルートIDへのリンクと保存日時が含まれるが、`<metadata>` にはルート名のみを残し、リンク・保存日時は取り込み時に削除している（個人が特定できる情報を配布物に残さないため）。
...
| F-305 | 全画面表示（映像+HUD 部分のみ。Fキーでも切替可） | 中 |

> **F-303 の意図**: 初心者が時速15kmで30分漕いでも実距離は7.5kmにしかならず、景色の変化が乏しい。倍率設定により「短時間でも遠くまで行った感」を演出でき、継続率（最重要KPI）に効く。

#### F-305 全画面表示（実装済み）

映像への没入感を高めるため、`ride-stage`（3D/2D映像 + HUD）を Fullscreen API で全画面化するボタンを追加した（`app/js/main.js` の `bindFullscreen()`）。

- 全画面中はサイドパネル（デバイス接続・走行操作ボタン）が物理的に見えなくなるため、**一時停止・終了ボタンを HUD 上にも複製**し、全画面のままでも操作できるようにしている
- 走行完了サマリーは `ride-stage` の外側にあるモーダルのため、全画面のまま終了すると隠れて見えなくなる。**終了時に自動で全画面を解除**してからサマリーを表示する
- Fallback2D の `<canvas>` はピクセル単位でサイズを保持しており、`window` の `resize` イベントで更新される。Fullscreen 切替時にこのイベントが確実に発火するとは限らないため、`fullscreenchange` イベント内で明示的に `resize` イベントを発火させて追従させている
- `F` キーでも切替可能（入力欄にフォーカスがある場合は無効化し、文字入力を優先する）
- Fullscreen API 非対応ブラウザ（Safari 等）ではボタン自体を隠す
...
以下 S-06〜S-08 は、サブスク経由（第7.5節）でのみ関係する追加要件。BYOK（自分のキーを貼る方式）には影響しない。

| ID | 要件 |
|---|---|
| S-06 | サブスク経由で使う系統A（Routes/Elevation/Static Maps）の実キーは**バックエンド（`server/`）の環境変数にのみ置き、クライアントへは一切渡さない**。クライアントに渡るのは系統B（Maps JavaScript API ローダー）用キーのみ |
| S-07 | 系統A用キーはサーバー間通信のため Referer ヘッダーが付かず、リファラ制限を掛けられない。その代わり系統B用キーとは**別枠で**日次クォータ上限・予算アラート（S-04/S-05 相当）を設定する |
| S-08 | バックエンドの各エンドポイントは、有効なライセンスキーを確認できない限りリクエストを拒否する（`server/lib/requireLicense.js`） |
...
| P-04 | **例外（サブスク利用時のみ）**: メールアドレスとライセンスキーは、有効性確認のためバックエンド（決済代行の Stripe を含む）に送信される。P-01 の対象（体重・体脂肪率・心拍・走行記録）はこの例外に含まれず、サブスク利用時も引き続き送信されない |
| P-05 | サブスクを使わない場合（BYOK または API キー未設定）は、これまでどおり一切のデータがブラウザ外に出ない |

### 7.5 サブスクリプション

自分の Google Maps API キーを用意しなくても、月額会員になれば Maps 連携機能（3D映像・ルート検索・地点指定ルートの標高・2Dフォールバックの背景地図）を使えるようにする課金経路。**BYOK（自分の API キーを設定画面に貼る、これまでの無料の使い方）とは独立した選択肢であり、置き換えるものではない。**

#### 7.5.1 なぜバックエンドが必要か

静的サイトである以上、API キーは原理的に隠せない（第7.3節冒頭）。しかし Maps 連携機能は、プロキシ可能かどうかで2系統に分かれる。

| 系統 | 対象API | プロキシ可否 | バックエンドの役割 |
|---|---|---|---|
| **A** | Routes API・Elevation API・Static Maps API | **可能**。自前の `fetch()` で呼んでいるため、サーバー経由に差し替えれば実キーはブラウザに一切出ない | 実キーを完全に秘匿する |
| **B** | Maps JavaScript API ローダー・3D Tiles・Street View | **不可**。SDK 内部が直接 Google を叩く仕様で、コード側から横取りできない | 「認可（未課金者には渡さない）」と「即時失効性」のみ提供。渡した後のキーはブラウザの開発者ツールで見える |

系統Bで配布したキーが見えてしまうのは実装の工夫では解消できない Maps JavaScript API の仕様上の制約であり、そのためリファラ制限（S-02）を必ず併用し、漏洩時にすぐローテーションできる体制（キー2本構成、S-06/S-07）を取る。

#### 7.5.2 バックエンド構成

`server/`（このリポジトリ内、`app/` とは別に Vercel へ単独デプロイ）に5つのエンドポイントを持つ。

| エンドポイント | 系統 | 処理 |
|---|---|---|
| `POST /api/maps/routes` | A | Routes API へ代理リクエスト |
| `GET /api/maps/elevation` | A | Elevation API へ代理リクエスト |
| `GET /api/maps/staticmap` | A | Static Maps 画像を代理取得してそのまま返却 |
| `POST /api/maps/key` | B | ライセンスキーの有効性を確認できた場合のみ、系統B用キーを返す |
| `GET /api/subscribe/complete` | — | Stripe Checkout 完了後、ライセンスキーを新規発行する |

このうち Maps 関連の4エンドポイントは共通のミドルウェア（`server/lib/requireLicense.js`）でライセンスキーの有効性を確認してから処理する（未設定なら401、無効なら403）。`/api/subscribe/complete` はライセンスキーを発行する側のエンドポイントのためこの確認の対象外で、代わりにStripeの完了済みCheckout Sessionであることを根拠に認可する。Google Cloud 側では、系統A用・系統B用に**別々のAPIキーを2本発行する**（系統Aはサーバー間通信のため Referer ヘッダーが付かず、リファラ制限キーと併用できないため）。

#### 7.5.3 決済

決済・税務・解約は **Stripe** に委ねる。理由:

- Checkout（決済ページ）・Customer Portal（解約・カード変更）が標準搭載されており、自前で決済UIや督促(dunning)ロジックを実装せずに済む（個人開発者が決済基盤を初めて持つ際の一番の事故要因）
- 顧客・サブスクリプションの状態をAPIで直接照会できるため、ライセンスキー相当の仕組みを自前で薄く実装できる（`server/lib/licenses.js`）
- 個人開発でも導入実績・ドキュメントが豊富

Stripeにはライセンスキー発行機能が無いため、Checkout完了後にバックエンド（`server/api/subscribe/complete.js`）がランダムなトークンを発行し、そのSHA-256ハッシュをStripe Customerのmetadataに保存して以後の検証に使う。ライセンスキーの形式は `vc_<Stripeカスタマーid>_<トークン>` で、顧客IDを含むことでStripe側の検索インデックス待ちなしに直接照会できる。

利用者はStripeでの決済完了後、Payment Linkのリダイレクト設定によりアプリへ自動的に戻り、ライセンスキーが自動保存される（`app/js/main.js` の `bindLicenseSetup`）。メールアドレス・ライセンスキーを手動入力する導線も残しており、別端末での利用や自動反映に失敗した場合はそちらでも有効化できる。

#### 7.5.4 障害時のふるまい

本アプリの既存の哲学（BLE/API失敗時は必ずフォールバックし、走行を止めない）をそのまま踏襲する。新しい失敗モードは発明しない。

| 状況 | ふるまい |
|---|---|
| バックエンド未デプロイ・不通 | 系統A・系統Bとも自動的に BYOK（自分の API キー）にフォールバック。BYOK も無ければ従来どおり 2D フォールバック・勾配0%で継続 |
| ライセンスキーが無効・期限切れ | 同上（バックエンドが403を返し、クライアント側でBYOKへフォールバック） |
| ライセンスキーもAPIキーも無い | ルート検索・標高取得は使えないが、同梱プリセットGPX・自作GPXインポート・BLE・カロリー計算・体重記録は引き続き無料で使える（これらはAPIキー非依存） |

#### 7.5.5 価格・法的事項

価格、および特定商取引法ページに載せる氏名/屋号・住所・返金ポリシーは**未確定**（第12章）。テンプレートを `docs/legal/` に用意しているが、実データは事業者本人が確定させる必要がある。
...
| A-05 | サブスクのバックエンドが不通・ライセンス無効と判定された場合も、クラッシュせず BYOK へフォールバックする（第7.5.4節） |
...
| L-06 | **バックエンド経由でのAPIアクセス提供**（第7.5節のサブスク） | 系統Aのプロキシ、系統Bのキー配布が Google Maps Platform 利用規約上の「利用制限」「第三者への提供」条項に抵触しないか、Phase B（実デプロイ・課金開始）着手前に必ず確認する。本書はこの論点の指摘に留め、結論は出していない |

> **注記**: 本章は調査時点の公開情報に基づく整理であり、法的助言ではない。特に L-01 の「一時的キャッシュの許容範囲」と L-06 の「バックエンド経由でのAPIアクセス提供の可否」については、実装・課金開始前に [Google Maps Platform Service Specific Terms](https://cloud.google.com/maps-platform/terms/maps-service-terms) の原文を確認すること。
...
| R-09 | 走行中の通信断・BLE切断 | 中 | 記録の継続と自動再接続（第7.6節） |
| R-10 | Google Maps Platform の料金体系変更 | 低 | コスト監視を継続。3Dタイルのセッション課金が変更された場合は映像方式の再評価が必要 |
| R-11 | サブスク基盤（Vercel/Stripe）の障害・未デプロイ | 中 | BYOK への自動フォールバック。無料導線（プリセットGPX・BLE等）は影響を受けない（第7.5.4節） |
...
| 3 | **利用者 = 本人のみの個人利用（BYOK）に加え、2026-08 以降はサブスク経由で不特定多数への公開を前提とする**よう変更した。第2.2節のAPIコスト試算は1ライドあたりの単価であり利用者数にほぼ比例して増えるため、サブスク会員数が増えた場合は日次クォータ・予算アラート（第7.3節 S-04/S-05/S-07）を利用者数に応じて見直す必要がある | 第2.2節、第7.3節、第7.4節、第7.5節、第8章 |
| 4 | **映像方式 = 3Dタイル主軸 + Street View スポット利用のハイブリッド** | 第2章、第6.2節 |
| 5 | **サブスク価格・特定商取引法上の事業者情報（氏名/屋号・住所・返金ポリシー）は未確定**。`docs/legal/` にテンプレートを用意し、実データの確定と Stripe 側の商品登録（Phase B）を待っている | 第7.5節、`docs/legal/` |
